...
DOCKER_IMAGE=docker.nbm.vn/vgasoft-"$BUILD_TAG":"$BUILD_NUMBER"
ENV_FILE=/data/env/dev.vgasoft.vn/.env.production # Default to dev environment
CONTAINER_NAME=vgasoft-"$BUILD_TAG"

if [ "$BUILD_TAG" = "main" ]; then
  ENV_FILE=/data/env/vgasoft.vn/.env.production # Production environment file
elif [ "$BUILD_TAG" = "test" ]; then
  ENV_FILE=/data/env/test.vgasoft.vn/.env.production # Staging environment file
fi
